...
def train(args, snapshot_path):
    ...
    db_train = BraTS2019(base_dir=train_data_path, datalist_path=dataset_list, split='train', num=None,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            # Generate the soft pseudo label
            soft_pesudo_1 = sharpening(pred_1[args.labeled_bs:])
            soft_pesudo_2 = sharpening(pred_2[args.labeled_bs:])

            # Generate the mask from the two pseudo labels
            mask = (soft_pesudo_1 * soft_pesudo_2 > 0.5).float()
            mask = torch.sum(mask, dim=1, keepdim=True)

            # Calculate the mutual loss
            mutual_dist = losses.softmax_mse_loss(output_1[args.labeled_bs:], output_2[args.labeled_bs:])
            loss_mutual = torch.sum((1-mask)*mutual_dist)/(2*torch.sum(1-mask)+1e-16)

            # Uncertainty map calculation
            variance_aux1 = torch.sum(kl_distance(
                torch.log(pred_1[args.labeled_bs:]), soft_pesudo_1), dim=1, keepdim=True)
            exp_variance_aux1 = torch.exp(-variance_aux1)
            variance_aux2 = torch.sum(kl_distance(
                torch.log(pred_2[args.labeled_bs:]), soft_pesudo_2), dim=1, keepdim=True)
            exp_variance_aux2 = torch.exp(-variance_aux2)

            # Calculate the intra loss
            consistency_dist_aux1 = (
                soft_pesudo_1 - pred_1[args.labeled_bs:]) ** 2
            ...
            consistency_dist_aux2 = (
                soft_pesudo_2 - pred_2[args.labeled_bs:]) ** 2
            ...
            loss_inter = (consistency_loss_aux1 + consistency_loss_aux2)/2

            consistency_weight = get_current_consistency_weight(iter_num//150)
            consistency_loss = loss_inter + loss_mutual
            ...
            logging.info(
                'iteration %d : loss : %f, supervised_loss: %f, loss_mutual: %f, loss_inter: %f' %
                (iter_num, loss.item(), supervised_loss.item(), loss_mutual.item(), loss_inter.item()))
